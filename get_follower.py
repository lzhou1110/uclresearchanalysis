...
def load_friends_dictionary():
    with open ('friends_dictionary.dat', 'rb') as friends_dictionary_file:
        friends_dictionary = pickle.load(friends_dictionary_file)
#     print('Loaded {} user-friends relationships'.format(len(friends_dictionary)))
    return friends_dictionary

def save_friends_dictionary(friends_dictionary): 
    with open('friends_dictionary.dat', 'wb') as friends_dictionary_file:
...
def main():
    print('Load all tweets')
    filepaths = [
        'temp/givenchy',
        'temp/stephen_hawking', 
        'temp/nyc_attack',
#        'temp/florida_shooting'
    ]
    
    for file_path in filepaths:
        with open (file_path + '/user_unique.dat', 'rb') as all_unique_tweets_file:
            tweets_dataframe = pickle.load(all_unique_tweets_file)
        print('Loaded {} tweets'.format(len(tweets_dataframe)))

        print('RP', len(tweets_dataframe[tweets_dataframe.source_type == 'RP']))
        print('RT', len(tweets_dataframe[tweets_dataframe.source_type == 'RT']))
        print('Potential Candidates', len(tweets_dataframe[tweets_dataframe.source_type == '']))  
        ...
        all_suspicious_users = tweets_dataframe[tweets_dataframe.source_type == '']
        print('Number of users we need to crawl the friends: {}'.format(len(all_suspicious_users)))
        found_users = friends_dictionary.keys()
        print('Number of users we already crawl: {}'.format(len(found_users)))
        need_to_crawl = set(all_suspicious_users.user_id) - set(found_users)
